...
void generate_cross_section_texture(Cross_Section &cs);
...
void initScene(int argc, char *argv[])
{
  ...
  generate_cross_section_texture(cs_l);
  generate_cross_section_texture(cs_r);
  generate_cross_section_texture(cs_bar);
  generate_cross_section_texture(cs_support);
...
int generate_cross_section_single(Cross_Section_Vertex &csv, Cross_Section_Buffer &buffer, SECTION_DIRECTION sd, int skip)
{
  ...
  for (; i < num_vertices; i += skip)
  {
    ...
    switch (sd) {
      case Y:
        push_side(p0, p1, p4, p5, right, index, csv.right, cross_section_right_index);
        push_side(p1, p2, p5, p6, up, index, csv.up, cross_section_up_index);
        push_side(p2, p3, p6, p7, left, index, csv.left, cross_section_left_index);
        push_side(p3, p0, p7, p4, down, index, csv.down, cross_section_down_index);
        break;
      case X:
        right = f.tangent;
        left = -right;
        push_side(p4, p5, p7, p6, right, index, csv.right, cross_section_right_index);
        push_side(p1, p2, p5, p6, up, index, csv.up, cross_section_up_index);
        push_side(p3, p2, p0, p1, left, index, csv.left, cross_section_left_index);
        push_side(p3, p0, p7, p4, down, index, csv.down, cross_section_down_index);
        break;
      case Z:
        up = f.tangent;
        down = -up;
        push_side(p0, p1, p4, p5, right, index, csv.right, cross_section_right_index);
        push_side(p4, p5, p7, p6, up, index, csv.up, cross_section_up_index);
        push_side(p2, p3, p6, p7, left, index, csv.left, cross_section_left_index);
        push_side(p3, p2, p0, p1, down, index, csv.down, cross_section_down_index);
        break;
      default:
        break;
    }
  ...
  set_one_vao_basic(pipelineProgram, csv.right.vertex, csv.right.color, buffer.right.vao);
  set_one_vao_basic(pipelineProgram, csv.up.vertex, csv.up.color, buffer.up.vao);
  set_one_vao_basic(pipelineProgram, csv.left.vertex, csv.left.color, buffer.left.vao);
  set_one_vao_basic(pipelineProgram, csv.down.vertex, csv.down.color, buffer.down.vao);
